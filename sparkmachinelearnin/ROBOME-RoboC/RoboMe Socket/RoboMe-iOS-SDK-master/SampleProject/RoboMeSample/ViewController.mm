...
//        NSData *welcomeData = [message dataUsingEncoding:NSUTF8StringEncoding];
    ...
    else if ([cmd containsString:@"SENTIMENT"]){
        NSLog(@"Command: %@",cmd);
        ...
        NSLog(@"snetimental data %@",lastWord);
        ...
        if ([lastWord isEqualToString:@"POSITIVE"]) {
            ...
            [self play:@"Hey seems to be good. What is the reason"];
            
        }else if ([lastWord isEqualToString:@"VERY POSITIVE"]){
            NSLog(@" very     positive");
            
            [self play:@"Awesome you seems to be fantastic. what happened"];
            
        }else if([lastWord isEqualToString:@"NEGATIVE"]){
            
            NSLog(@"negative");
            [self play:@"Some thing seems to be wrong what happened?"];
            
        }else if([lastWord isEqualToString:@"VERY NEGATIVE"]){
            
            NSLog(@"very negative");
            [self play:@"Don't worry every thing will be alright. what happened?"];
            
        }else if([lastWord isEqualToString:@"NEUTRAL"]){
            
            NSLog(@"neutral");
            [self play:@"nothing to worry"];
        }
    ...
    else if ([cmd containsString:@"FIRE"]){
        NSLog(@"Fire block");
        [self play:@"Fire has been occured in the house. Call emergency"];
    }
    ...
    //accelerometer
        ...
        triggerImageURL = [[NSBundle mainBundle] pathForResource:@"Lema" ofType:@"bmp"];
...
//                cv::Mat networkImage = [guitarImage CVGrayscaleMat];
    ...
    }else if([msg containsString:@"hospital"]){
